...
`;
    ...
    buildUnixHookCommand(eventType, tool = '') {
        ...
        if (eventType === 'claude_finished') {
            return `sh -c '[ -z "$CODEAGENTSWARM_COMMIT_MODE" ] && (curl -X POST http://127.0.0.1:${devPort}/webhook -H "Content-Type: application/json" -d "{\\"type\\":\\"${eventType}\\",\\"terminalId\\":\\"$(echo \${CODEAGENTSWARM_CURRENT_QUADRANT:-0})\\"}" --silent --fail --connect-timeout 1 --max-time 2 2>/dev/null || curl -X POST http://127.0.0.1:${prodPort}/webhook -H "Content-Type: application/json" -d "{\\"type\\":\\"${eventType}\\",\\"terminalId\\":\\"$(echo \${CODEAGENTSWARM_CURRENT_QUADRANT:-0})\\"}" --silent --fail --connect-timeout 1 --max-time 2 2>/dev/null) || true'`;
        } else if (eventType === 'confirmation_needed') {
            return `sh -c '(curl -X POST http://127.0.0.1:${devPort}/webhook -H "Content-Type: application/json" -d "{\\"type\\":\\"${eventType}\\",\\"terminalId\\":\\"$(echo \${CODEAGENTSWARM_CURRENT_QUADRANT:-0})\\",\\"tool\\":\\"${tool}\\"}" --silent --fail --connect-timeout 1 --max-time 2 2>/dev/null || curl -X POST http://127.0.0.1:${prodPort}/webhook -H "Content-Type: application/json" -d "{\\"type\\":\\"${eventType}\\",\\"terminalId\\":\\"$(echo \${CODEAGENTSWARM_CURRENT_QUADRANT:-0})\\",\\"tool\\":\\"${tool}\\"}" --silent --fail --connect-timeout 1 --max-time 2 2>/dev/null) || true'`;
        } else {
            return `sh -c '(curl -X POST http://127.0.0.1:${devPort}/webhook -H "Content-Type: application/json" -d "{\\"type\\":\\"${eventType}\\",\\"terminalId\\":\\"$(echo \${CODEAGENTSWARM_CURRENT_QUADRANT:-0})\\"}" --silent --fail --connect-timeout 1 --max-time 2 2>/dev/null || curl -X POST http://127.0.0.1:${prodPort}/webhook -H "Content-Type: application/json" -d "{\\"type\\":\\"${eventType}\\",\\"terminalId\\":\\"$(echo \${CODEAGENTSWARM_CURRENT_QUADRANT:-0})\\"}" --silent --fail --connect-timeout 1 --max-time 2 2>/dev/null) || true'`;
        }
    }
