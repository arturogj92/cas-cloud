/**
 * Kimi Conversation Search Service
 *
 * Searches and retrieves conversation history from the Kimi Code CLI. Drop-in
 * sibling of the Codex/Antigravity/opencode search services: a default-exported
 * class exposing
 ...
 * (+ isKimi / agent:'kimi').
 *
 * Storage details live in kimi-conversation-reader.js (plain JSON/JSONL under
 * ~/.kimi-code). Although Kimi stores sessions grouped per working directory,
 * this service treats the list GLOBALLY (project filter accepted but unused),
 * matching the unfiltered get-index skeleton — the lesson from opencode
 * (2026-07-04): a loaded list filtered tighter than the skeleton makes rows
 * appear and then VANISH, breaking the history "no reshuffle" guarantee.
 */

const reader = require('./kimi-conversation-reader');
...
// Kimi's auto-generated placeholder title for fresh sessions.
const NEW_SESSION_TITLE = /^New Session/i;

class KimiConversationSearchService {
    normalizeString(str) {
    ...
    buildDisplayText(text, maxLength = 100) {
        ...
        if (!cleaned) return 'Kimi Session';
    ...
    /**
     * Resolve the display text for a session. A user-set title (isCustomTitle,
     * settable in Kimi with /title) always wins; the "New Session" placeholder
    ...
    resolveDisplayText(session) {
        ...
        if (title && (session.isCustomTitle || !NEW_SESSION_TITLE.test(title))) {
        ...
        return this.buildDisplayText(title || 'Kimi Session');
    }

    /**
     * Get recent Kimi conversations.
     * @param {number} limit
        ...
        try {
            ...
            for (const session of sessions) {
                try {
                    ...
                    console.error(`[Kimi] Error building conversation ${session.sessionId}:`, err.message);
                }
            ...
            console.log(`[Kimi Conversation Search] Found ${conversations.length} conversations in ${Date.now() - startTime}ms`);
            return conversations;
        } catch (error) {
            console.error('Error reading Kimi conversation history:', error);
            return [];
        }
    }
    ...
    /**
     * Content search is not indexed for Kimi yet (parity with Antigravity/opencode).
     */
    ...
    async getConversationContent(sessionId, projectDir = null) {
        try {
            ...
            console.log(`[Kimi getConversationContent] Found ${messages.length} messages`);
            return messages;
        } catch (error) {
            console.error('[Kimi getConversationContent] Error:', error);
            return [];
        }
    }
}

module.exports = KimiConversationSearchService;
